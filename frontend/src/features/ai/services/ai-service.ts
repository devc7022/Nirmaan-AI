...
export const aiService = {
  ...
  parseAttendance: async (data: AttendanceAIRequest): Promise<AttendanceAIResponse> => {
    ...
    try {
      const systemInstruction = `You are an AI assistant for BumbleBrick AI Construction Management. Extract attendance details from natural language input. Return ONLY a valid JSON object matching this structure:
{
  "workerName": "string",
  "siteName": "string",
...
Ensure no markdown code block text wraps the output, only valid raw JSON.`;
  ...
  sendChatMessage: async (data: ChatRequest): Promise<ChatResponse> => {
    ...
    try {
      const systemInstruction = `You are BumbleBrick AI Workforce Assistant, an expert AI for construction site management, labor tracking, safety compliance, site attendance, and cost optimization. Give professional, helpful, accurate markdown responses. Keep responses structured and concise.`;
  ...
  generateReport: async (): Promise<DailyReport> => {
    ...
    try {
      ...
      const systemInstruction = `You are an expert construction site auditor for BumbleBrick AI. Generate a professional Daily Workforce Report in markdown. Include Executive Overview, Site Breakdown, Trade Distributions, Logged Hours, and Actionable Safety Recommendations.`;
...
function mockDailyReport(): DailyReport {
  ...
  const content = `# 🏗️ BumbleBrick AI - Daily Workforce Summary Report
**Date:** ${todayStr}
...
*Report generated automatically by BumbleBrick AI Workforce Assistant.*`;
